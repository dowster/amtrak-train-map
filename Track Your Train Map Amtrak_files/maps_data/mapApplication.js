...
function resetTimeout() {
  ...
  timeoutInterval = setTimeout(sessionTimeout, configData.session_timeout * 1000);
}
...
function showCompatWindow() {
  ...
  $(".compatmode_content").css({
    top:
      ($("#compatmode").outerHeight(false) -
        $(".compatmode_content").height()) /
        2 +
      "px",
    left:
      ($("#compatmode").outerWidth(false) - $(".compatmode_content").width()) /
        2 +
      "px",
  });
  $(window).resize();
...
function autoCompleteCallback(req, res) {
    ...
    for (t in __$$_td) {
      if (__$$_td[t].properties.TrainNum.indexOf(req.term) == 0) {
        ...
        if (typeof matches.matched[term] == "undefined") {
          matches.trains.push({
            label: term + "  " + __$$_td[t].properties.RouteName,
            id: term,
          });
    ...
    for (s in stations) {
      ...
      if (req.term.length >= 3) {
        ...
        if (
          ...
          var term = buildStationTerm(stations[s]);
          if (typeof matches.matched[term] == "undefined") {
            matches.station_names.push({
              label: term,
              id: s,
              sortOrder: stations[s].Name,
            });
            matches.matched[term] = 1;
          }
        }
  ...
  if (
    ...
    (matches.station_codes.length > 0 ||
      matches.station_city.length > 0 ||
      matches.station_names.length > 0)
  ) {
  ...
  res(results.length > 0 ? results : [{ id: 0, label: lblnoResults }]);
}
...
function retrieveStationData(data) {
  ...
  $.each(data.features, function (i, d) {
    ...
    var rightContent =
      ...
      '<div class="if_station_code">' +
      ID +
      "</div>" +
      '<div class="if_station_icon_' +
      iconType +
      '">';
    ("</div>");
      ...
      fixup: function (t, d) {
        ...
        if (t.left_content.length > 50) {
          ...
          $(lc).attr(
            "style",
            "font-size: " +
              (parseInt(lc.css("font-size"), 10) - 1) +
              "px !important;"
          );
        }
        ...
        $(d)
          .find("div.train")
          .on("click", function (i, el) {
            var ID = $(this).attr("id");
            var arr = [];
            arr.push(__$$_td[ID]);
            filterTrains(arr);
            map.setZoom(configData.item_zoom);
    ...
    var hoverContent =
      cityState.toUpperCase() + "&nbsp;&nbsp;(" + ID.toUpperCase() + ")";
    var hoverType = "station_";
    if (hoverLength >= 26) hoverType += "long";
    else if (hoverLength >= 17) hoverType += "med";
    else hoverType += "short";
    ...
    stations[ID].events = {};
    stations[ID].events.click = new google.maps.event.addDomListener(
      stations[ID].marker,
      "click",
      function (event) {
        ...
        if (map.IsDefaultView() || map.ZoomLevelChanged()) {
          ...
          // VV 7/1/2014: fix bug in IE8 - check map zoom with configData.item_zoom and redraw infoWindow if different
          if (map.zoom != configData.item_zoom) {
            map.setZoom(configData.item_zoom);
          } else {
            map.redrawInfoWindow(500);
          }
    ...
    stations[ID].events.mouseover = new google.maps.event.addDomListener(
      stations[ID].marker,
      "mouseover",
      function (event) {
    ...
    stations[ID].events.mouseout = new google.maps.event.addDomListener(
      stations[ID].marker,
      "mouseout",
      function (event) {
        if (lastHover === null) {
  ...
  if (configData.debug) log("Stations Fetched: " + stationCount);

  // Load active trains after stations have finished.
  setTimeout(__$$_ratl, configData.train_layer_delay);
}
...
function setStationStatus(ID) {
  ...
  if (!stationStatus.arrivalStatus.length)
    extraContentArr +=
      '<div class="if_station_extra_status_row bold text_center">No Scheduled Trains</div>';
  // LOOP THROUGH ARRIVALS ...
  $.each(stationStatus.arrivalStatus, function (i, status) {
    $.each(status.stationStatus, function (index, field) {
      // LOOP THROUGH STATIONSTATUS FIELDS ...
      ...
      if (matches) {
        ...
        var hasPostArr =
          typeof field.postarr != "undefined" && field.postarr != null;
        var hasEstArr =
          typeof field.estarr != "undefined" && field.estarr != null;
        var hasSchArr =
          typeof field.scharr != "undefined" && field.scharr != null;
        var hasSchDep =
          typeof field.schdep != "undefined" && field.schdep != null;
        var noStatus = field.postcmnt == "ARRIVED";
        var notLast = true;
        var blue =
          hasPostArr || hasCancellation
            ? " text_grey"
            : " text_blue underlined pointer train";
        var blueDate =
          hasPostArr || hasCancellation ? " text_grey" : " text_blue";
        ...
        if (hasPostArr || hasEstArr || hasSchArr || hasSchDep) {
          ...
          if (status.stationStatus.showmarker == "Y") {
            extraContentArr +=
              status.showmarker == "Y"
                ? '<div class="if_station_extra_status_row"><div class="if_station_extra_status_station ' +
                  blue +
                  '" id=' +
                  status.ActiveTrainId +
                  ">" +
                  status.TrainNum +
                  '</div><div class="if_station_extra_status_station ' +
                  blueDate +
                  ' trainDate">' +
                  moment(status.datetoSort).format("MMMM D, YYYY") +
                  "</div>"
                : '<div class="if_station_extra_status_row"><div class="if_station_extra_status_station float_left text_black" id=' +
                  status.ActiveTrainId +
                  ">" +
                  status.TrainNum +
                  '</div><div class="if_station_extra_status_station ' +
          ...
          if (hasCancellation) {
            extraContentArr +=
              ...
              moment(schDate).format(configData.time_format) +
              " " +
              field.tz +
              "T</div>";
            extraContentArr +=
              '<div class="if_station_extra_status_comment float_right' +
              grey +
              '">' +
              formatComment(field.schcmnt) +
              '<div class="if_station_status_' +
              evalStationStatus(field.schcmnt) +
              '"></div></div></div>';
          } else {
            if (hasPostArr)
              extraContentArr +=
                ...
                '<div class="if_station_extra_status_times float_left text_grey">' +
                moment(field.postarr).format(configData.time_format) +
                " " +
                field.tz +
                "T</div>";
            else if (hasEstArr) {
              ...
              extraContentArr +=
                ...
                '<div class="if_station_extra_status_times float_left text_black">' +
                moment(field.estarr).format(configData.time_format) +
                " " +
                field.tz +
                "T</div>";
            } else if (hasSchArr && !hasPostArr && !hasEstArr)
              extraContentArr +=
                '<div class="if_station_extra_status_label float_left text_black">Scheduled</div>' +
                '<div class="if_station_extra_status_times float_left text_black">' +
                moment(schDate).format(configData.time_format) +
                " " +
                field.tz +
                'T</div><div class="if_station_extra_status_comment float_right text_black">&nbsp;<div class="if_station_status_none">&nbsp;</div></div>';
            else
              extraContentArr +=
                '<div class="if_station_extra_status_label float_left text_black">Scheduled</div>' +
                '<div class="if_station_extra_status_times float_left text_black">' +
  ...
  $.each(stationStatus.departureStatus, function (i, status) {
    $.each(status.stationStatus, function (index, field) {
      ...
      if (matches) {
        ...
        var hasCancellation =
          hasSchCmnt && field.schcmnt.toUpperCase() === "CANCELED";
        var hasPostDep =
          typeof field.postdep != "undefined" && field.postdep != null;
        var hasEstDep =
          typeof field.estdep != "undefined" && field.estdep != null;
        var hasSchDep =
          typeof field.schdep != "undefined" && field.schdep != null;
        var noStatus = field.postcmnt == "DEPARTED";
        var notLast = true;
        var blue =
          hasPostDep || hasCancellation
            ? " text_grey"
            : " text_blue underlined pointer train";
        var blueDate =
          hasPostDep || hasCancellation ? " text_grey" : " text_blue";
        ...
        if (hasPostDep || hasEstDep || hasSchDep) {
          ...
          if (hasCancellation) {
            extraContentDep +=
              '<div class="if_station_extra_status_label float_left text_grey">Scheduled</div>' +
              '<div class="if_station_extra_status_times float_left text_grey">' +
              moment(field.schdep).format(configData.time_format) +
              " " +
              field.tz +
              "T</div>";
            extraContentDep +=
            ...
            if (hasPostDep)
              extraContentDep +=
                ...
                '<div class="if_station_extra_status_times float_left text_grey">' +
                moment(field.postdep).format(configData.time_format) +
                " " +
                field.tz +
                "T</div>";
            else if (hasEstDep) {
              ...
              extraContentDep +=
                '<div class="if_station_extra_status_label float_left text_black">Estimated</div>' +
                '<div class="if_station_extra_status_times float_left text_black">' +
                moment(field.estdep).format(configData.time_format) +
                " " +
                field.tz +
                "T</div>";
...
function getStationStatus(stationCode) {
  ...
  var newURL = configData.vAllTrainsURL;
        ...
        $.each(status.stationStatus, function (index, field) {
          ...
          if (matches) {
            ...
            if (fields.code == stationCode) {
              status.datetoSort =
                typeof fields.schdep !== "undefined"
                  ? fields.schdep
                  : fields.scharr;
              var hasPostArr =
                typeof fields.postarr != "undefined" && fields.postarr != null;
              var hasEstArr =
                typeof fields.estarr != "undefined" && fields.estarr != null;
              var hasSchArr =
                typeof fields.scharr != "undefined" && fields.scharr != null;
              var hasSchDep =
                typeof fields.schdep != "undefined" && fields.schdep != null;
              if (hasPostArr || hasEstArr || hasSchArr)
                arrivalStatus.push(status);
              else if (
                typeof status.stationStatus.postcmnt !== "undefined" &&
                status.stationStatus.postcmnt === "ARRIVED"
              )
                arrivalStatus.push(status);
              var hasPostDep =
                typeof fields.postdep != "undefined" && fields.postdep != null;
              var hasEstDep =
                typeof fields.estdep != "undefined" && fields.estdep != null;
              var hasSchDep =
                typeof fields.schdep != "undefined" && fields.schdep != null;
              if (hasPostDep || hasEstDep || hasSchDep)
                departureStatus.push(status);
...
function resetStationStatusWindow(d, isArrival) {
  ...
  if (isArrival) {
    ...
    if (
      $(d).find("#expand_status_button_arr").attr("class").indexOf("minus") >= 0
    ) {
      $(d)
        .find("#expand_status_button_arr")
        .removeClass("if_station_extra_content_status_minus_arr");
      $(d)
        .find("#expand_status_button_arr")
        .addClass("if_station_extra_content_status_plus_arr");
    }
  } else {
    ...
    if (
      $(d).find("#expand_status_button_dep").attr("class").indexOf("minus") >= 0
    ) {
      $(d)
        .find("#expand_status_button_dep")
        .removeClass("if_station_extra_content_status_minus_dep");
      $(d)
        .find("#expand_status_button_dep")
        .addClass("if_station_extra_content_status_plus_dep");
    }
  }
}

function loadStationDats() {
  try {
    $.get(_$_sal_5, {})
      .done(function (d) {
        var x12 = d.trim();
        ...
        retrieveStationData(
          JSON.parse(
            securityHelper.decrypt(
              x12.substring(0, x12.length - masterSegment),
              securityHelper
                .decrypt(x12.substr(x12.length - masterSegment), publicKey)
                .split("|")[masterSegment - 88]
            )
...
function resetAllMap() {
  ...
  resetMap();
  resetGlobals();
  map.setCenter(configData.map_center);
  map.setZoom(configData.map_options.zoom);
...
function retrieveRouteList(data) {
  ...
  $.getJSON(configData.route_properties_url, function (data) {
    $.each(data.features, function (i, d) {
      $.each(d.properties.NAME.split(" / "), function (i, n) {
        var r = resolver.routes[n.toLowerCase()];
        if (typeof r == "undefined" && configData.debug) {
    ...
    for (r in resolver.routes) {
      ...
      if (typeof rt == "undefined" && configData.debug) {
        log("No resolver for " + r);
        continue;
      }
      if (typeof rt.boxes == "undefined" && configData.debug) {
...
function initialize() {
  ...
  // callback to use "?", not "&"
  $.getJSON(configData.route_list_url, {
    dataType: "json",
    ...
    .fail(function (x) {
      ...
      if (configData.debug) log("Failed to retrieve route list data.");
    });
  ...
  $.each(configData.images, function (i, s) {
    ...
    dynImage.onload = function () {
      ...
      if (typeof s[1] == "object") {
        ...
        if (configData.isIE8 && typeof result != "undefined") {
          newStyle += result;
        }
      } else {
        var styleName = s[1];
  ...
  var mapInitZoom;
  if ($(document).width() > 2000) mapInitZoom = 6;
  else if ($(document).width() > 1400) mapInitZoom = 5;
  else if ($(document).width() > 1200) mapInitZoom = 5;
  else mapInitZoom = 4;

  configData.map_options.zoom = mapInitZoom;

  map = new google.maps.Map($("#map_canvas")[0], configData.map_options);
  map.SearchType = "";
  map.IsDefaultView = function () {
    return map.zoom == configData.map_options.zoom && zoomEventCount == 1;
  };
  map.ZoomLevelChanged = function () {
    return lastZoomLevel != map.zoom;
  };
  map.MinorZoomTriggered = function () {
    return Math.abs(configData.item_zoom - map.zoom) <= 2;
  };
  map.redraw = function () {
  ...
  map.redrawInfoWindow = function (v) {
    var timeout = typeof v === "undefined" ? configData.redraw_timeout : v;
  ...
  map.collapseInfoWindow = function () {
    if (lastInfoWindow != null && !lastInfoWindow.collapsed) {
      ...
      if (
        btnArr.length > 0 &&
        btnArr.hasClass("if_station_extra_content_status_minus_arr")
      ) {
        btnArr.click();
      ...
      if (
        btnDep.length > 0 &&
        btnDep.hasClass("if_station_extra_content_status_minus_dep")
      ) {
        btnDep.click();
  ...
  map.hideClusterInfoWindows = function () {
    ...
    if (markerClusterer != null) {
      $.each(markerClusterer.getClusters(), function (i, c) {
        ...
        if (
          c.getMarkers().length >= configData.clustering_options.minimumClusterSize
        ) {
  ...
  map.updateStationMarkers = function () {
    ...
    for (s in stations) {
      if (level > configData.zoom_level_resolver[stations[s].MapZmLvl]) {
        var stationIcon = calculateIconScaleFromZoomLevel(map.getZoom());
    ...
    for (var i in __$$_td) {
      if (typeof __$$_td[i] !== "undefined") {
        ...
        if (f === null) {
          if (configData.debug) {
            log(
              "Deleted: Train " + __$$_td[i].properties.TrainNum + "[" + i + "]"
            );
          }
          delete __$$_td[i];
        }
  ...
  map.refocusLastSelectedMarker = function () {
    ...
    if (
      __$$_lastSelTrDa !== null &&
      typeof __$$_td[__$$_lastSelTrDa] === "undefined"
    ) {
      resetAllMap();
      return;
    }
  ...
  map.events = {
    ...
    center_changed: function () {
      ...
      if (!configData.restrict_bounds) return;
      if (configData.strict_bounds.contains(map.getCenter())) {
    ...
    idle: function () {
      ...
      //            $.each(markerClusterer.getClusters(), function(i, c) {
      //                if (c.getMarkers().length > configData.clustering_options.minimumClusterSize) {
      //                    $.each(c.getMarkers(), function(i, m) {
      //                        if (m === __$$_td[__$$_lastSelTrDa].marker) {
    ...
    idle_once: function () {
      ...
      google.maps.event.addListener(
        map,
        "center_changed",
        map.events.center_changed
      );

      google.maps.event.addListener(
        map,
        "zoom_changed",
        map.events.zoom_changed
      );

      google.maps.event.addListener(
        map,
        "bounds_changed",
        map.events.bounds_changed
      );
    },
  };
  ...
  var mapReset = $.parseHTML(
    '<div><div class="map_reset_button" id="map_reset_button" title="Reset">&nbsp;</div></div>'
  );
  mapReset[0].index = 1;
  ...
  map.mapTypes.set(configData.map_type_id, customMapType);
  ...
  autoCompleter = $("#search_field").autocomplete({
    ...
    focus: function (event, ui) {
      $(".ui-autocomplete")
        .find("hr")
        .removeClass("ui-menu-item-wrapper ui-state-active");
  ...
  autoCompleter.data("ui-autocomplete")._renderItem = function (ul, item) {
    if (item.appendHR) {
      return $("<li>")
        .append(
          "<a>" + item.label + "</a>" + "<hr class='ui-menu-item-wrapper-hr'/>"
        )
        .appendTo(ul);
    } else {
      return $("<li>")
  ...
  searchWindow = new iwsInfoWindow({
    ...
    HasMultipleResults: function () {
      if (sortedTrains == null) return false;
    ...
    minimize: function () {
      ...
      if (
        $("#expand_search_button").attr("class") ==
        "if_train_search_results_minus"
      ) {
        $(".if_train_search_results").hide();
        $("#expand_search_button").attr(
          "class",
          "if_train_search_results_plus"
        );
      }
    },
    setOffset: function () {
      ...
      if ($(searchWindow.div_).is(":visible")) {
        var offsets = [
          0, 0, 0, 0, 3, 2, 1, 0.5, 0.2, 0.1, 0.05, 0.03, 0.02, 0.01, 0.004,
          0.002,
        ];
        var position = new google.maps.LatLng(
          lastInfoWindow.position.lat(),
          lastInfoWindow.position.lng()
        );
...
function openSearchWindow(label, trains) {
  ...
  var midContent =
    '<div class="if_train_search_results_minus" id="expand_search_button">&nbsp</div>';

  var setupSearchButton = function (t, d) {
    $(d)
      .find("#expand_search_button")
      .hover(
        function () {
          if (this.className.indexOf("minus") >= 0) {
  ...
  var statusResolver = {
    /*
         ...
         + moment(t.properties.EventDT).format(configData.time_format) + t.properties.EventTZ + "T<br />"
         + moment(t.properties.EventSchDp).format(configData.time_format) + t.properties.EventTZ + "T"
         + '</div>';
    ...
    Arrival: [
      ...
      function (t) {
        ...
        var retVal =
          '<div class="float_left text_grey search_arrdep">' +
          "Arrived<br />Scheduled Arrival" +
          "</div>" +
          '<div class="float_left text_grey">' +
          moment(at).format(configData.time_format) +
          t.properties.EventTZ +
          "T<br />" +
          moment(tt).format(configData.time_format) +
          t.properties.EventTZ +
          "T" +
          "</div>";
    ...
    "Estimated Arrival": [
      ...
      function (t) {
        ...
        if (nextStation.HasPostedStatus || nextStation.HasEstimatedStatus) {
          retVal +=
            moment(nextStation.scharr || nextStation.schdep).format(
          ...
          if (nextStation.HasEstimatedStatus)
            retVal +=
              "<br />" +
              moment(nextStation.estarr).format(configData.time_format) +
              nextStation.tz +
              "T";
        } else {
          var tt = t.properties.EventSchAr || t.properties.EventSchDp;
          ...
          retVal +=
            moment(tt).format(configData.time_format) + t.properties.EventTZ + "T";

          if (nextStation.HasEstimatedStatus)
            retVal +=
              "<br />" +
              moment(t.properties.EventDT).format(configData.time_format) +
              t.properties.EventTZ +
              "T";
        }
        retVal += "</div>";
    ...
    Predeparture: [
      ...
      function (t) {
        var retVal =
          '<div class="float_left text_grey search_arrdep">Scheduled Departure</div>' +
          '<div class="float_left text_grey">' +
          moment(t.schdep).format(configData.time_format) +
          t.tz +
          "T" +
          "</div>";
        return retVal;
  ...
  $.each(sortedTrains, function (i, t) {
    ...
    if (evnt != null && !preD && evnt == "" && code.length > 0) {
    ...
    var nextStation = getNextStationDetail(t);
    var stationLabel =
      typeof stations[cc] != "undefined" ? stations[cc].City : cc;
    if (typeof nextStation != "undefined" && !nextStation.HasPostedStatus)
      stationLabel =
        typeof stations[nextStation.code] != "undefined"
          ? stations[nextStation.code].City
          : nextStation.code;
    extraContent +=
      '<div class="if_train_search_result" id="search_train_' +
      t.properties.ID +
      '">' +
      '<div class="float_left">' +
      '<div class="text_blue underlined pointer">' +
      t.properties.TrainNum +
      " " +
      label +
      "</div>";
    if (
      typeof t.properties.StatusMsg != "undefined" &&
      t.properties.StatusMsg.indexOf("SERVICE DISRUPTION") >= 0
    ) {
      extraContent +=
      ...
      extraContent +=
        ...
        "<br />".repeat(breaks) +
        "</div>" +
        '<div class="float_right text_bottom if_search_results_flag if_train_status_' +
        trainStatus +
        '">&nbsp;</div>' +
        '<div class="float_right text_grey text_bottom">' +
        formatComment(cmnt.toLowerCase()) +
        "</div>" +
        "</div>" +
        "</div>";
    }
  ...
  if (list != null) {
    if (DynamicMapsEngineLayerSupport()) {
      $.each(list, function (i, gx) {
        ...
        if (typeof ls.zIndex != "undefined") gxs.zIndex = ls.zIndex;
      });
    } else {
...
function resetRouteLines() {
  if (lastSelectedFeatures == null) return;
...
function resetMap() {
  ...
  $.each(__$$_td, function (i, t) {
    if (typeof t == "undefined" || typeof t.properties == "undefined") return;
...
function filterTrains(tl) {
  ...
  for (t in __$$_td) {
    if (typeof __$$_td[t] == "undefined") continue;
...
function selectRoute(r) {
  ...
  /*
	...
	routeLayer.getSubLayers()[0].set({
		 sql: sqlStmt,
		 cartocss:configData.higlightedLineStyle
	});
*/
  ...
  if (r.OffsetY) OffsetY = parseFloat(r.OffsetY) + parseFloat(OffsetY);

  var OffsetX = parseFloat(r.bounds.getSouthWest().lng());
  if (r.OffsetX) OffsetX = parseFloat(r.OffsetX) + parseFloat(OffsetX);
...
function selectTrain(t) {
  ...
  if (t.length == 1) {
    ...
    if (t[0].properties.RouteName.indexOf("/") > 0) {
      displayRouteForTrainSearch(
        resolver.routes[x.split("/")[0].trim().toLowerCase()]
      );
    } else {
      displayRouteForTrainSearch(resolver.routes[x.toLowerCase()]);
    }
    map.fitBounds(
      new google.maps.LatLngBounds().extend(t[0].marker.getPosition())
    );
    map.setZoom(configData.item_zoom);
...
function getNextStationDetail(t) {
  ...
  var eventStation = $.grep(t.train_status, function (e, i) {
    if (t.properties.TrainState != "Predeparture") {
      return (
        e.code ==
        (ec != t.train_status[0].code ? ec : t.train_status[parseInt(1)].code)
      );
    } else {
  ...
  if (typeof eventStation == "undefined") {
    if (t.TrainState != "Predeparture")
      log(
        "eventStation not defined for train #" +
          t.properties.TrainNum +
          ": " +
          t.properties.EventCode +
          ": ID :" +
          t.properties.ID
      );
    return {
      HasPostedStatus: false,
      HasPostedArrivedStatus: false,
      HasPostedDepartedStatus: false,
      HasEstimatedStatus: false,
      code: t.properties.EventCode,
    };
  }
  ...
  eventStation.HasPostedDepartedStatus =
    stationHasPostedDepartedStatus(eventStation);
  eventStation.HasEstimatedStatus = stationHasEstimatedStatus(eventStation);
  if (!eventStation.HasPostedArrStatus) return eventStation;
  ...
  var nextStation =
    typeof t.train_status[parseInt(idx) + 1] != "undefined"
      ? t.train_status[parseInt(idx) + 1]
      : t.train_status[idx];
  if (typeof nextStation != "undefined" && typeof nextstation != null) {
    nextStation.HasPostedStatus = stationHasPostedStatus(nextStation);
    nextStation.HasPostedArrivedStatus =
      stationHasPostedArrivedStatus(nextStation);
    nextStation.HasPostedDepartedStatus =
      stationHasPostedDepartedStatus(nextStation);
...
function displayStationLayer() {
  var mz = map.getZoom();
  for (s in stations) {
    if (mz > configData.zoom_level_resolver[stations[s].MapZmLvl])
      stations[s].marker.setMap(map);
  }
}

function retryTrainsLayer() {
  if (++__$$_tdr > configData.max_load_retries) {
...
function retryStationsLayer() {
  if (++__$$_sdr > configData.max_load_retries) {
    // cw: Show timeout or error message on screen.
    showTrackingUnavailable(true);
  }
  setTimeout(loadStationDats, 2000 * Math.pow(2, __$$_sdr - 1));
}
  ...
  if (typeof c != "undefined") {
    ...
    if (t.properties.TrainState != "Predeparture") {
      if (c == "canceled") {
        ...
        if (
          minMatch != null &&
          (typeof minMatch[2] == "undefined" || minMatch[2] == "") &&
...
function __$$_rtd(data) {
  try {
    ...
    /* if (typeof data.error != "undefined" && configData.debug) {
    ...
    __$$_tdr = 0;
    if (configData.debug) log("Active Trains Fetched: " + data.features.length);
    ...
    if (data.features.length > 0) {
      ...
      if (
        !configData.debug &&
        jstz.determine().name().toLowerCase().indexOf("america") != -1
      ) {
        if ((dtNow - dtLast) / 1000 / 60 > configData.lastUpdateDiff) {
          showTrackingUnavailable(true);
          isUnavailable = true;
        } else {
          if (isUnavailable) {
    ...
    $.each(data.features, function (i, d) {
      ...
      if (
        typeof d.properties.RouteName == "undefined" ||
        typeof d.properties.CMSID == "undefined"
      ) {
        if (configData.debug)
          log(
            "No route associated with " +
      ...
      if (
        d.properties.hasOwnProperty("Heading") &&
        d.properties.Heading !== null
      ) {
      ...
      for (x = 1; x <= configData.max_stations; x++) {
        if (
          typeof d.properties["Station" + x] != "undefined" &&
          d.properties["Station" + x] != null
        ) {
          __$$_td[ID].train_status.push(
            $.parseJSON(d.properties["Station" + x])
          );
        }
        ...
        // data.
        if (!configData.debug) delete d.properties["Station" + x];
      }

      // cw: Count BACKWARDS to find last station with posted TS.
      var x;
      for (x = __$$_td[ID].train_status.length - 1; x >= 0; x--) {
        if (typeof __$$_td[ID].train_status[x] == "undefined") continue;
      ...
      if (!srvDis) {
        ...
        if (d.properties.TrainState === "Completed") {
          extraContent += moment(d.properties.ViewStn2).format(
            configData.time_format
          );
        } else
          extraContent += moment(d.properties.LastValTS).format(
            configData.time_format
          );
        ...
        extraContent +=
          getNextStationDetail(__$$_td[ID]).tz +
          "T" +
          "</div>" +
        ...
        for (x = 0; x < __$$_td[ID].train_status.length; x++) {
          ...
          var hasPostDep =
            typeof station_status.postdep != "undefined" &&
            station_status.postdep != null;
          var hasPostArr =
            typeof station_status.postarr != "undefined" &&
            station_status.postarr != null;
          var hasEstArr =
            typeof station_status.estarr != "undefined" &&
            station_status.estarr != null;
          var hasEstDep =
            typeof station_status.estdep != "undefined" &&
            station_status.estdep != null;
          ...
          var stationblack =
            notLast || hasSchCmnt ? " text_grey" : " text_black";
          var stationgrey = notLast || hasSchCmnt ? " text_grey" : "";

          var stationDetailDisplay =
            station_detail.City == ""
              ? station_status.code
              : station_detail.City +
                ", " +
                station_detail.State +
                (station_detail.Name != null && station_detail.Name.length > 0
                  ? " - " + station_detail.Name
                  : "");
          //Bug: 3209 - CBN should be ignored.
          if (stationDetailDisplay == "CBN") continue;
          ...
          extraContent +=
            ...
            "_status_" +
            x +
            '" class="if_train_extra_status_row">';
          extraContent +=
            '<div class="if_train_extra_status_station' +
            blue +
            '" id="station_' +
            station_status.code +
            '">' +
            stationDetailDisplay +
            "</div>";

          if (x === __$$_td[ID].train_status.length - 1) {
            extraContent +=
              '<div class="stationDate if_train_extra_status_station' +
              stationblue +
              '">' +
              moment(station_status.scharr).format("MMMM D, YYYY") +
              "</div>";
          } else {
            extraContent +=
              '<div class="stationDate if_train_extra_status_station' +
              stationblue +
              '">' +
              moment(station_status.schdep).format("MMMM D, YYYY") +
              "</div>";
          }
          ...
          if (x == 0) {
            if (hasPostDep) {
              extraContent +=
                '<div class="if_train_extra_status_label float_left' +
                grey +
                '">' +
                (hasPostDep ? "Departed" : "Scheduled Departure") +
                "</div>" +
                '<div class="if_train_extra_status_times float_left' +
                black +
                '">' +
                moment(station_status.schdep).format(configData.time_format) +
                " " +
                station_status.tz +
                "T</div>";
            } else if (hasEstDep) {
              ...
              extraContent +=
                ...
                black +
                '">' +
                moment(station_status.estdep).format(configData.time_format) +
                " " +
                station_status.tz +
                "T</div>";
            } else {
            ...
            extraContent +=
              ...
              moment(station_status.postarr).format(configData.time_format) +
              " " +
              station_status.tz +
              "T</div>";
          } else if (x != 0) {
            if (!noStatus) {
              extraContent +=
                ...
                (hasEstArr
                  ? "<br />" +
                    (__$$_td[ID].properties.TrainState != "Completed"
                      ? "Estimated Arrival"
                      : "Arrived")
                  : "") +
                "</div>" +
                ...
                black +
                '">' +
                moment(station_status.scharr || station_status.schdep).format(
                  configData.time_format
                ) +
                "&nbsp;" +
                station_status.tz +
                ...
                (hasEstArr
                  ? "<br />" +
                    moment(station_status.estarr).format(configData.time_format) +
                    "&nbsp;" +
                    station_status.tz +
                    "T"
                  : "") +
          ...
          if (noStatus) {
            ...
            var c = hasSchCmnt
              ...
              : hasPostArr
              ? station_status.postcmnt
              : station_status.estarrcmnt;

            if (typeof c === "undefined") c = "";

            //It's a little convoluted but the X==0 takes care of showing the scheduled dep.
            extraContent +=
              '<div class="if_train_extra_status_comment float_right' +
              grey +
              '">' +
              formatComment(
                hasPostArr || hasSchCmnt || (hasEstDep && x == 0)
                  ? c.toLowerCase()
                  : hasEstArr
                  ? "<br />" + c.toLowerCase()
                  : ""
              ...
              (hasPostArr || hasEstArr || hasSchCmnt
                ? '<div class="if_train_extra_status_ts_' +
                  evalTrainStatus(__$$_td[ID], c) +
                  '"></div>'
                : "") +
              "</div>";
          }
        ...
        if (
          moment(d.properties.OrigSchDep).dayOfYear() != moment().dayOfYear()
        ) {
          midContent +=
            '<div class="float_left">' +
        ...
        if (nextStation.HasEstimatedStatus) {
          tComment = formatComment(
            nextStation.estarrcmnt || nextStation.estdepcmnt
          );
        }
        ...
        if (evnt == "Estimated Departure") evnt = "Estimated Arrival";
        ...
        if (d.properties.TrainState == "Active") {
          if (
            typeof tComment == "undefined" &&
            configData.debug &&
            configData.log_type == "verbose"
          ) {
          ...
          if (
            ...
            if (
              evnt != "Arrival" ||
              d.properties.EventCode != d.properties.DestCode
            ) {
              if (tsRec.length > 0) {
                ...
                midContent +=
                  ...
                  (nextStation.HasEstimatedStatus ? "Estimated" : "Scheduled") +
                  " Arrival" +
                  "</div>";
              } else {
                midContent += "<br />";
            ...
            /* 
                     ...
                     *
                     ...
                     + moment(d.properties.LastValTS).format(configData.time_format) + d.properties.EventTZ + "T</span></div></div>";
                     */
            d.train_status = __$$_td[d.properties.ID].train_status;
            var nextStation = getNextStationDetail(d);

            midContent += '<div class="text_blue float_left bold">';

            if (nextStation.HasPostedStatus) {
              if (
                typeof d.properties.EventDT != "undefined" &&
                d.properties.EventDT.length > 0
              ) {
                midContent +=
                  moment(d.properties.EventDT).format(configData.time_format) +
                  d.properties.EventTZ +
                  "T";
              }
            } else {
              if (nextStation.HasEstimatedStatus)
            ...
            midContent +=
              ...
              (nextStation.HasEstimatedStatus
                ? tComment.toLowerCase()
                : "no status") +
              '<div class="if_train_extra_status_ts_' +
              trainStatus +
              '"></div>' +
          ...
          if (tsRec.length > 0) {
            ...
            midContent +=
              ...
              '<span class="text_black">' +
              nextStationDisplay(nData, tsRec[0].code) +
              "</span><br />";
            ...
            if (tsRec.length > 2) {
              ...
              if (typeof tsRec[0].estdep !== "undefined") {
                midContent +=
                  '<div class="text_blue float_left if_train_info_mid_hspacer bold">Estimated Departure</div>' +
                  '<div class="text_blue float_left bold">' +
                  moment(tsRec[0].estdep).format(configData.time_format) +
                  tsRec[0].tz +
                  "T";
                hasED = true;
              } else {
                midContent +=
                  '<div class="text_blue float_left if_train_info_mid_hspacer bold">Scheduled Departure</div>' +
                  '<div class="text_blue float_left bold">' +
                  moment(tsRec[0].schdep).format(configData.time_format) +
                  tsRec[0].tz +
                  "T";
              }
            } else {
              midContent +=
                '<div class="text_blue float_left if_train_info_mid_hspacer bold">Scheduled Arrival</div>' +
                '<div class="text_blue float_left bold">' +
                moment(tsRec[0].scharr).format(configData.time_format) +
                tsRec[0].tz +
                "T";
            }
        ...
        if (
          d.properties.TrainState === "Completed" ||
          d.properties.TrainState === "Predeparture"
        ) {
        ...
        rightContent +=
          ...
          "</div>&nbsp;" +
          '<div class="if_train_direction_' +
          heading +
          '"></div>&nbsp;' +
          '<div class="if_train_status_' +
          trainStatus +
          '"></div>' +
          "</div>";
      } else {
      ...
      if (d.properties.RouteName != null)
        leftContent += "&nbsp;&nbsp;" + d.properties.RouteName.toUpperCase();
              ...
              $(statB)
                .hover(
                  function () {
                    if (this.className.indexOf("minus") >= 0) {
                      this.className =
                        "if_train_extra_content_status_minus_hover";
                    } else {
                      this.className =
                        "if_train_extra_content_status_plus_hover";
                    }
                  },
                  function () {
                    if (this.className.indexOf("minus") >= 0) {
                ...
                .click(function () {
                  ...
                  if (this.className.indexOf("minus") >= 0) {
                    ...
                    if (t.last_station >= 1) {
                      statDiv.scrollTop(
                        $(d)
                          .find("#train_" + ID + "_status_" + t.last_station)
                          .position().top
                      );
                    }
                    lastInfoWindow.collapsed = false;
      ...
      var hoverLength = (d.properties.TrainNum + " " + d.properties.RouteName)
        .length;
      var hoverPrefix =
        hoverLength > configData.train_hover_long ? "train_" : "train_short_";
      // cw: For display ONLY.
      var hoverContent = d.properties.TrainNum;
      if (d.properties.RouteName != null)
        hoverContent += "&nbsp;&nbsp;" + d.properties.RouteName.toUpperCase();
      // cw: xxx - Move to configData.js ASAP!
      ...
      __$$_td[ID].events.click = google.maps.event.addListener(
        __$$_td[ID].marker,
        "click",
        function (e) {
          __$$_lastSelTrDa = ID;
          ...
          if (
            map.IsDefaultView() ||
            map.ZoomLevelChanged() ||
            searchWindow.HasMultipleResults()
          ) {
            if (map.MinorZoomTriggered()) {
              map.redrawInfoWindow(500);
            }
      ...
      __$$_td[ID].events.mouseover = new google.maps.event.addDomListener(
        __$$_td[ID].marker,
        "mouseover",
        function (event) {
          if (lastHover) {
      ...
      __$$_td[ID].events.mouseout = new google.maps.event.addDomListener(
        __$$_td[ID].marker,
        "mouseout",
        function (event) {
          if (lastHover === null) {
    ...
    var setClusterDivImage = function (c, i) {
      ...
      cdiv.css(
        ...
        parseFloat(cdiv.css("top").replace("px", "")) +
          (deltaY *
            (images["cluster-hover"].height - images["cluster-bg"].height)) /
            2 +
          "px"
      );
      cdiv.css(
        ...
        parseFloat(cdiv.css("left").replace("px", "")) +
          (deltaX *
            (images["cluster-hover"].width - images["cluster-bg"].width)) /
            2 +
          "px"
      );
    ...
    refreshInterval = setTimeout(
      __$$_ratl,
      configData.train_refresh_interval * 1000
    );
...
function __$$_ratl() {
  ...
  $.get(_$_tal_4, {}).done(function (d) {
    try {
      ...
      /*MasterSegment is the length of the string at the end of the encrypted data that contains the secret key
                ...
                7. publicKey - the public key that we obtain

                securityHelper : variable name for the Security object in the Helper.js file - that file has the calls to actually decrypt the data.
      ...
      var json = JSON.parse(
        securityHelper.decrypt(
          ...
          securityHelper
            .decrypt(dd.substr(dd.length - masterSegment), publicKey)
            .split("|")[masterSegment - 88]
        )
      );
      //Bug fix 1: 9/17/2020
  ...
  $(window).resize(function () {
    $(".timeout_content").css({
      top:
        ($("#timeout").outerHeight() - $(".timeout_content").height()) / 2 +
        "px",
      left:
        ($("#timeout").outerWidth() - $(".timeout_content").width()) / 2 + "px",
    });

    $(".unavailable_content").css({
      top:
        ($("#unavailable").outerHeight() - $(".unavailable_content").height()) /
          2 +
        "px",
      left:
        ($("#unavailable").outerWidth() - $(".unavailable_content").width()) /
          2 +
        "px",
    });
    ...
    $(".compatmode_content").css({
      top:
        ($("#compatmode").outerHeight() - $(".compatmode_content").height()) /
          2 +
        "px",
      left:
        ($("#compatmode").outerWidth() - $(".compatmode_content").width()) / 2 +
        "px",
    });
  });
  $(window).resize();
...
function getRouteLayer(sqlStmt) {
  if (sqlStmt == "") {
    var url = configData.baseRouteLayerURL;

    $.getJSON(url, function (data) {
      routeLayer = map.data.addGeoJson(data);
      map.data.setStyle({
        strokeColor: configData.baseLayerLineStyleStrokeColor,
        strokeWeight: configData.baseLayerLineStyleStrokeWeight,
      });
    });
  } else {
    ...
    var newURL = configData.baseRouteLayerURL + sqlStmt;
    $.getJSON(newURL, function (data) {
      routeLayer = map.data.addGeoJson(data);
      map.data.setStyle({
        strokeColor: configData.highlightedLayerLineStyleStrokeColor,
        strokeWeight: configData.highlightedLineStyleStrokeWeight,
      });
    });
  }
}
...
function calculateIconScaleFromZoomLevel(zLevel) {
  var newIcon = configData.stationIconRegular; //base value
  //Scale revised  Per BT:3607 MB
  if (zLevel > 4 && zLevel <= 7) newIcon = configData.stationIconRegular;
  if (zLevel >= 8 && zLevel <= 10) newIcon = configData.stationIconMedium;
  if (zLevel > 10) newIcon = configData.stationIconLarge;
